...
PATH_FILE = os.path.join(ROOT, "out", "result.csv")  # model,size,iter,....metrics
...
def main():
    ...
    data = data.melt(id_vars=["size", "model", "iter"], var_name="metrics").query(
        "metrics != 'n_all'"
    )
    ...
    # summary
    data.groupby(["size", "model", "metrics"]).median().reset_index().sort_values(
        ["metrics", "size", "model"]
